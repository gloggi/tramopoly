...
const results = plv8.execute('SELECT auth.uid() AS user_id, group_id_for(auth.uid()) AS group_id, role_for(auth.uid()) AS role, m.group_id AS message_group_id, m.reactions as reactions FROM messages m WHERE id=$1 LIMIT 1', [message_id])
const { user_id, group_id, role, message_group_id, reactions } = results[0]
