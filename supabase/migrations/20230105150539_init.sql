...
-- Moved here from the start of the first "real" migration, because this does not seem to be
-- part of the changes I made for our application. So this should just be applied when running
-- locally, but not when setting up a new project on Supabase online.
alter table if exists "auth"."sso_sessions" drop constraint "sso_sessions_session_id_fkey";

alter table if exists "auth"."sso_sessions" drop constraint "sso_sessions_sso_provider_id_fkey";

alter table if exists "auth"."sso_sessions" drop constraint "sso_sessions_pkey";
