...
export const stationVisitsDB = (
  start = new Date(0),
  end = new Date(new Date().getTime() + READ_AHEAD)
) => {
  return db
    .collection("stationVisits")
    .where("time", ">", start)
    .where("time", "<=", end)
    .orderBy("time");
};
export const jokerVisitsDB = (
  start = new Date(0),
  end = new Date(new Date().getTime() + READ_AHEAD)
) => {
  return db
    .collection("jokerVisits")
    .where("time", ">", start)
    .where("time", "<=", end)
    .orderBy("time");
};
export const mrTChangesDB = (
  start = new Date(0),
  end = new Date(new Date().getTime() + READ_AHEAD)
) => {
  return db
    .collection("mrTChanges")
    .where("time", ">", start)
    .where("time", "<=", end)
    .orderBy("time");
};
...
export function changeGroupOperator(abteilungId, operatorId) {
  ...
  if (!operatorId) {
    ...
    return db
      .collection("abteilungen")
      .doc(abteilungId)
      .update({ operator: db.collection("users").doc(operatorId) });
  }
}
...
export function setGlobalMessage(type, title, message) {
  return db
    .collection("settings")
    .doc("settings")
    .update({ message: { type, title, message } });
}
...
export function setActiveCall(operatorId, callerId) {
  ...
  return db
    ...
    .doc(operatorId)
    .update({
      activeCall: callerId ? db.collection("users").doc(callerId) : null,
    });
}
...
export function addStationVisit(groupId, stationId) {
  ...
  return db
    .collection("stationVisits")
    .doc(time.toLocaleTimeString("de-CH") + " " + groupId + " " + stationId)
    .set({
...
export function addJokerVisit(groupId, jokerId) {
  ...
  return db
    .collection("jokerVisits")
    .doc(time.toLocaleTimeString("de-CH") + " " + groupId + " " + jokerId)
    .set({
      group: db.collection("groups").doc(groupId),
...
export function addMrTChange(groupId, mrTChangeData) {
  ...
  return db
    .collection("mrTChanges")
    .doc(time.toLocaleTimeString("de-CH") + " " + groupId)
    .set({
      ...mrTChangeData,
      group: db.collection("groups").doc(groupId),
      time,
...
export function setMrTShouldCallOperator(mrTId) {
  ...
  return db
    .collection("mrTChanges")
    .doc(mrTId)
    .update({ shouldCallOperator: true });
}
...
export async function createCheckpoint(date) {
  ...
  return db
    .collection("checkpoints")
    .doc(date.toLocaleTimeString("de-CH"))
    .set(checkpoint);
}
...
async function calculateCheckpoint(checkpointDate) {
  ...
  const stationsMap = stations.reduce(
    (map, station) => map.set(station.id, station),
    new Map()
  );
  const jokers = await getValueOnce(jokersDB());
  const jokersMap = jokers.reduce(
    (map, joker) => map.set(joker.id, joker),
    new Map()
  );
  const stationVisits = (
    await getValueOnce(stationVisitsDB(new Date(0), checkpointDate))
  ).map((stationVisit) => ({
    ...stationVisit,
  ...
  const jokerVisits = (
    await getValueOnce(jokerVisitsDB(new Date(0), checkpointDate))
  ).map((jokerVisit) => ({
    ...jokerVisit,
    station: jokersMap.get(jokerVisit.station.id),
    group: groupsMap.get(jokerVisit.group.id),
  }));
  const mrTChanges = await getValueOnce(
    mrTChangesDB(new Date(0), checkpointDate)
  );
  ...
  return calculateCheckpointData(
    groups,
    stationVisits,
    jokerVisits,
    mrTChanges,
...
function findUserByPhone(phone) {
  return new Promise((resolve) => {
    db.collection("users")
      .where("phone", "==", phone)
      .onSnapshot((snapshot) => resolve(snapshot));
  });
}
...
export function bindLoggedInUser(vm, member, cancelCallback, readyCallback) {
  auth.onAuthStateChanged((loggedInUser) => {
    if (loggedInUser) {
      vm.$bind(member, db.collection("users").doc(loggedInUser.uid), {
        maxRefDepth: 3,
      })
        .then(readyCallback)
  ...
  next((vm) => {
    bindLoggedInUser(vm, "loggedInOperator", null, () => {
      if (
        !vm.loggedInOperator ||
        !(
          vm.loggedInOperator.role === "operator" ||
          vm.loggedInOperator.role === "admin"
        )
      ) {
