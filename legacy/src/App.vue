  ...
  <div id="app">
    <div class="level">
      <div class="level-left">
        <span v-if="userIsLoggedIn && user.scoutName" class="level-item"
          >Willkommä, {{ user.scoutName }}.</span
        >
        <span v-else-if="userIsLoggedIn" class="level-item">Willkommä.</span>
        <a v-if="userIsLoggedIn" class="level-item" @click="signout"
          >Uusloggä</a
        >
        <router-link v-else :to="{ name: 'login' }" class="level-item"
          >Iiloggä</router-link
        >
        <router-link
          v-if="userIsAdmin"
        ...
        >
        <router-link
          v-else-if="userIsLoggedIn && !userIsOperator"
          :to="{ name: 'index' }"
          class="level-item"
          >{{ user.group.name }}</router-link
        >
        <router-link
          v-if="userIsOperator"
          :to="{ name: 'zentrale' }"
          class="level-item"
          >Zentralä</router-link
        >
        <router-link :to="{ name: 'map' }" class="level-item"
          >Jokers</router-link
        >
        <router-link
          v-if="userIsAdmin"
    ...
    <router-view
      ...
      :station-owners="stationOwners"
      :mr-t-location="mrTLocation"
      :now="now"
    >
      <b-message
        slot="message"
...
export default {
  ...
  computed: {
    ...
    userIsOperator() {
      return (
        this.userIsLoggedIn &&
        (this.user.role === "operator" || this.user.role === "admin")
      );
    },
    userIsAdmin() {
    ...
    checkpoint() {
      return this.checkpoints.length
        ? this.checkpoints.find((cp) => cp.time)
        : null;
    },
    allGroupsAndStationOwners() {
      if (
        ...
        (this.groups.length &&
          this.groups.some(
            (group) => !(group.abteilung && group.abteilung.id)
          )) ||
        !this.$firestoreRefs.checkpoints ||
        !this.$firestoreRefs.settings ||
        !this.$firestoreRefs.stationVisits ||
        !this.$firestoreRefs.jokerVisits ||
        !this.$firestoreRefs.mrTChanges
      ) {
  ...
  methods: {
      ...
      return new Promise((resolve) => {
        auth.onAuthStateChanged((firestoreUser) => {
          if (firestoreUser) {
            ...
            bindUserById(this, "user", firestoreUser.uid).then(async () => {
    ...
    bindRestrictedCollections() {
      const checkpointDate = this.checkpoint
        ? this.checkpoint.time.toDate()
        : new Date(0);
      return Promise.all([
    ...
    support() {
      this.$buefy.snackbar.open({
        message:
          "Wänn öppis nöd aazäigt wird, tuän mal d Siitä noi ladä 🔄 Wänns dänn immär nonig gaht, lüüt am Cosinus aa: Null Sibä Nüün, Drüü Acht Sächs, Sächs Sibä, Null Sächs",
        position: "is-top",
        indefinite: true,
      });
    },
