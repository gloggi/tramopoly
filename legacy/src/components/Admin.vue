  ...
  <div class="columns is-multiline">
    ...
    <div class="column is-full has-text-centered">
      <div v-if="loggedInOperatorBusy" class="card">
        <div class="card-content">
          <header class="card-header-title is-centered">
            Aktivä Aruäf: {{ loggedInAdmin.activeCall.scoutName }}
          </header>
          ...
          <div class="columns">
            <div class="column">
              <b-field grouped group-multiline>
                <b-field label="Start"
                  ><b-input readonly :value="gameStart" expanded
                /></b-field>
                <b-field label="Ändi"
                  ><b-input readonly :value="gameEnd" expanded
                /></b-field>
              </b-field>
              <b-field
                ><p class="control">
              ...
              <b-field label="Regischtriärig für Zentralä">
                <b-switch
                  type="is-danger"
                  :value="operatorGroupActive"
                  @input="(value) => setOperatorGroupAvailable(value)"
                >
                  <span v-if="operatorGroupActive" class="has-text-weight-bold"
                    >Aktiviert</span
                  >
            ...
            <div class="column">
              ...
              <b-field grouped>
                <p
                  class="control"
                  v-if="
                    mrTChanges && mrTChanges.length && !mrTShouldCallOperator
                  "
                >
                  <button
              ...
              <template v-if="message">
                ...
                <form @submit.prevent="setMessage">
                  <b-field grouped group-multiline>
                    <b-field
                      ><b-select name="type" v-model="selectedMessageType">
      ...
      <div class="card" v-if="abteilungenReady">
        <header class="card-header has-background-light">
          <span class="card-header-title title is-4"
            ><span class="icon is-medium" style="margin-right: 0.5em"
              ><img style="opacity: 0.7" :src="'/gloggi.svg'" /></span
            >Abteiligä</span
          >
        </header>
        <b-table
          class="has-content-vcentered"
          :data="abteilungen"
          striped
          hoverable
          :row-class="hasContentVcentered"
          default-sort="totalPoints"
          default-sort-direction="desc"
        >
          <template v-slot="props">
            <b-table-column field="name" label="Abteilig" sortable>
              <span v-if="props.row.id" class="icon is-medium"
                ><img
                  :title="props.row.name"
                  style="opacity: 0.7"
                  :src="'/' + props.row.id + '.svg'"
              /></span>
              <span class="has-text-weight-bold">{{ props.row.name }}</span>
            </b-table-column>
            <b-table-column field="saldo" label="Guäthabä" sortable>{{
              props.row.saldo
            }}</b-table-column>
            <b-table-column
              ...
              label="Immobiliä"
              sortable
              >{{ props.row.realEstatePoints }}</b-table-column
            >
            <b-table-column field="mrTPoints" label="Mr. T" sortable>{{
              props.row.mrTPoints
            }}</b-table-column>
            <b-table-column field="totalPoints" label="Total" sortable
            ...
            <b-table-column field="operator.scoutName" label="Telefonischt">
              <span>
                <b-select
                  @input="(value) => changeOperator(props.row.id, value)"
                  :value="props.row.operator.id"
                >
                  <option value=""></option>
                  <option
      ...
      <div class="card" v-if="users.length">
        ...
        <b-table
          class="has-content-vcentered"
          :data="usersWithMrTFlag"
          striped
          hoverable
          :row-class="hasContentVcentered"
        >
          <template v-slot="props">
            <b-table-column field="scoutName" label="Namä" sortable
              ><span
                ><span class="has-text-weight-bold">{{
                  props.row.scoutName
                }}</span
                ><b-tag
                  v-if="props.row.isCurrentlyMrT"
                  type="is-info"
              ...
              ></b-table-column
            >
            <b-table-column field="phone" label="Telefon" sortable>{{
              props.row.phone
            }}</b-table-column>
            <b-table-column
            ...
            >
              <span v-if="props.row.group && props.row.group.abteilung">
                <span v-if="props.row.group.abteilung.id" class="icon is-medium"
                  ><img
                    :title="props.row.group.abteilung.name"
                    style="opacity: 0.7"
                    :src="'/' + props.row.group.abteilung.id + '.svg'"
                /></span>
                <span>{{ props.row.group.abteilung.name }}</span>
              </span>
            </b-table-column>
            <b-table-column field="group.name" label="Gruppä" sortable
              ><span v-if="props.row.group">{{
            ...
            <b-table-column field="role" label="Rollä" sortable>
              <span>
                <b-select
                  @input="(value) => changeRole(props.row.id, value)"
  ...
  computed: {
    ...
    gameStart() {
      return (
        this.settings &&
        new Date(
          this.settings.gameEnd.toDate() - this.gameDuration
    ...
    gameEnd() {
      return (
        this.settings &&
        this.settings.gameEnd.toDate().toLocaleTimeString("de-CH")
      );
    },
    abteilungen() {
      const map = new Map();
      this.allGroups.forEach((group) => {
        ...
        if (!map.has(id)) {
          ...
          if (
            group.abteilung &&
            group.abteilung.operator &&
            group.abteilung.operator.id
          ) {
            operator = {
              name: group.abteilung.operator.scoutName,
              id: group.abteilung.operator.id,
            };
          }
          map.set(id, {
            id,
            name: group.abteilung.name,
            operator,
            saldo: 0,
            realEstatePoints: 0,
            mrTPoints: 0,
      ...
      map.forEach((abteilung) => {
        ...
        abteilung.realEstatePoints = Math.round(
          abteilung.realEstatePoints / abteilung.numGroups
        );
        abteilung.mrTPoints = Math.round(
          abteilung.mrTPoints / abteilung.numGroups
        );
        abteilung.totalPoints = Math.round(
          abteilung.totalPoints / abteilung.numGroups
        );
      });
    ...
    usersWithMrTFlag() {
      return this.users.map((user) => ({
        ...user,
        id: user.id,
        isCurrentlyMrT: user.group.id === this.mrTGroupId,
      }));
    },
    operatorGroupActive() {
    ...
    currentMrT() {
      return (
        [...this.mrTChanges].sort((a, b) => a.time.toDate() - b.time.toDate())[
          this.mrTChanges.length - 1
        ] || {}
      );
    },
    currentMrTActive() {
    ...
    minutesSinceLastActiveMrTChange() {
      return timeSinceLastActiveMrTChange(
        this.checkpoint,
        this.mrTChanges,
        this.now
      );
  ...
  methods: {
    ...
    markIfCallingGroup(group) {
      return this.loggedInAdmin &&
        ...
        this.loggedInAdmin.activeCall.group.id === group.id
        ? "is-active-call is-clickable has-content-vcentered"
        : "is-clickable has-content-vcentered";
    },
    setStartTimeToNow() {
    ...
    setMessage(submitEvent) {
      setGlobalMessage(
        submitEvent.target.elements.type.value,
        submitEvent.target.elements.title.value,
        submitEvent.target.elements.message.value
      );
    },
    ...
    confiscateMrT() {
      const groupId =
        (this.currentMrT.group && this.currentMrT.group.id) || "zentrale";
    ...
    onSort(field, order) {
      ...
      if (field === "time") {
        ...
        this.eventSorter = (a, b) =>
          dir * this.compareStrings(a.group.name, b.group.name);
      } else if (field === "type") {
        this.eventSorter = (a, b) =>
          dir *
          this.compareStrings(
            this.makeEventComparable(a),
            this.makeEventComparable(b)
          );
      }
    },
    compareStrings(a, b) {
    ...
    makeEventComparable(event) {
      return (
        (event.type === "station"
          ? "0_" + event.station.name
          : event.type === "joker"
          ? "1_" + event.station.name
          : "2") + event.time.toDate().getTime()
      );
    },
    createCheckpointToday() {
      ...
      const checkpointDate = new Date(
        ...
        today.getMonth(),
        today.getDate(),
        ...this.checkpointTime.match(/\d+/g).slice(0, 3),
        0
      );
