      ...
      <div v-if="operator" class="card">
        <div class="card-content">
          <b-message
            ...
            Minutä bi dä Zentralä aalütä. Bitte mäldäd oi bi oiäm
            Telefonischt.</b-message
          >
          <button
            v-if="!operatorBusy"
...
export default {
  ...
  beforeRouteEnter(to, from, next) {
    requireAuth(to, from, next, (vm) => () => {
      if (vm.loggedInUser.role === "operator")
        vm.$router.replace({ name: "zentrale" });
    });
  },
  computed: {
    groupId() {
      return (
        this.loggedInUser &&
        this.loggedInUser.group &&
        this.loggedInUser.group.id
      );
    },
    operatorName() {
    ...
    loggedInUserIsActiveCaller() {
      return (
        this.operatorBusy &&
        this.operator.activeCall.id === this.loggedInUser.id
      );
    },
    groupIsCurrentlyMrT() {
    ...
    callOperator() {
      setActiveCall(this.operatorId, this.loggedInUser.id).then(() => {
        if (this.loggedInUser.preferWhatsApp) {
          window.location =
            "https://api.whatsapp.com/send?phone=" + this.operatorPhone;
        } else {
