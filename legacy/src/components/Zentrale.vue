  ...
  <div class="columns is-multiline">
    ...
    >
      <div v-if="loggedInOperatorBusy" class="card">
        <div
          v-if="loggedInOperator.activeCall.id !== loggedInOperator.id"
          class="card-content"
        >
        ...
        <div v-else class="card-content">
          <header class="card-header-title is-centered">
            Aktuell wird ich als bsetzt aazäigt.
          </header>
          <button
            @click="clearActiveCall"
            class="button is-success is-outlined"
          >
            ☎️ Mich als frei aazäigä
          </button>
        </div>
      </div>
          ...
          <template v-slot="props">
            ...
            >
              <span class="icon is-medium"
                ><img
                  :title="props.row.abteilung.name"
                  style="opacity: 0.7"
                  :src="'/' + props.row.abteilung.id + '.svg'"
              /></span>
            </b-table-column>
            <b-table-column field="name" label="Gruppä" sortable
              ><span
              ...
              ></b-table-column
            >
            <b-table-column field="saldo" label="Saldo" sortable
              >{{ props.row.saldo }}.-</b-table-column
            >
            <b-table-column
  ...
  methods: {
    ...
    markIfCallingGroup(group) {
      return this.loggedInOperator &&
        ...
        this.loggedInOperator.activeCall.group.id === group.id
