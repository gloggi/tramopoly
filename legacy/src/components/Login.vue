    ...
    >
      <form v-on:submit.prevent="login">
        ...
        <b-field label="Abteilig">
          ...
          <b-input
            v-else-if="specifiedGroup.abteilung"
            v-model="specifiedGroup.abteilung.name"
            disabled
          />
...
export default {
  ...
  computed: {
    ...
    userIsAlreadyRegistered() {
      return (
        this.specifiedUser &&
        Object.prototype.hasOwnProperty.call(this.specifiedUser, "group")
      );
    },
    groups() {
  ...
  methods: {
    login() {
      this.specifiedGroupData = {
        name: this.groupName,
        abteilung: this.abteilung,
        active: true,
      };
      this.specifiedUserData = {
        phone: this.normalizedPhone,
        scoutName: this.scoutName,
        group: this.groupName,
        preferWhatsApp: this.preferWhatsApp,
        role: "",
      };
      auth
        .signInWithPhoneNumber(this.specifiedUserData.phone, this.appVerifier)
        .then((result) => {
    ...
    async verifyOtp() {
      ...
      if (!this.userIsAlreadyRegistered) {
        ...
        await setWhatsAppPreference(
          result.user.uid,
          this.specifiedUserData.preferWhatsApp
        );
      }
      this.$emit("login");
